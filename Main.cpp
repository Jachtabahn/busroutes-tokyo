...
std::vector<Region> loadRegions(std::string filename) {
    ...
    assertion(in != NULL, "Regions geojson file missing!");
...
struct KnapsackItem {
    ...
    KnapsackItem(const std::vector<std::string>& tokens) {
        for (int i = 0; i < tokens.size(); ++i) {
            if (tokens[i] == "RouteID") {
    ...
    KnapsackItem(std::string outputId, double cost, double benefits[MAX_BUSES]) {
...
std::vector<KnapsackItem*> loadRoutes(std::string filename) {
    FILE* in = fopen(filename.c_str(), "r");
    assertion(in != NULL, "KnapsackItem geojson file missing!");

    std::vector<KnapsackItem*> routes;
    while (fgets(line, MAX_LEN, in)) {
        std::vector<std::string> tokens = clean(line);
        if (tokens.size() >= 2 && tokens[0] == "type" && tokens[1] == "Feature") {
            // this is a route
            auto route = new KnapsackItem(tokens);
...
void computeBenefits(
    const std::vector<KnapsackItem*>& routes,
    ...
    for (int routeId = 0; routeId < routes.size(); ++routeId) {
        ...
        for (int age_group = 1; age_group <= 6; ++age_group) {
            ...
            for (int regionId = 0; regionId < regions.size(); ++regionId) {
                ...
                bool intersects = hasIntersection(route->polyline, region.polygon);
                if (!intersects) {
                    continue;
                }
...
int main() {
    ...
    std::vector<KnapsackItem*> routes = loadRoutes(ROUTE_JSON);
    ...
    computeBenefits(routes, regions, target_age_groups, tz_length);
    ...
    for (const auto& iter : routes) {
        fprintf(stderr, "KnapsackItem %s: Cost=%f\n", iter->outputId.c_str(), iter->cost);
    ...
    */
    ...
    for (int r = 0; r < routes.size() && budget_left > 0; ++r) {
        ...
        allocations[route->outputId] += 1;
        budget_left -= route->cost;
        fprintf(stderr, "selected %s, total_cost = %.2f / %.2f\n", route->outputId.c_str(), budget-budget_left, budget);
        ...
        auto new_route = new KnapsackItem(route->outputId, route->cost, ben);
